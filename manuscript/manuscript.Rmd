...
if (!is_docx) {
  ...
  tbl_var_summary %>% 
    ...
    insert_row("Predictors", "", "", "", after = 5) %>% 
    set_bold(1, everywhere) %>%
    set_bold(c(2, 6), everywhere) %>%
    set_bottom_border(c(2, 6), everywhere) %>%
    set_width(1) %>% 
    set_wrap(TRUE) %>% 
...
tbl_rmse_caption <- "Root mean squared error across models predicting physical violence and private civil liberties"
...
if (!is_docx) {
  ...
  tbl_rmse %>% 
    ...
    set_bold(1, everywhere) %>%
    set_bold(c(2, 9), everywhere) %>%
    set_bottom_border(c(2, 9), everywhere) %>%
    set_width(1) %>% 
    set_wrap(TRUE) %>% 
